...
function PageGuardModal({ arriveDate, departDate, guests, selectedRoom }) {
  ...
  return goodDate && goodGuests && goodRoom ? null : (
    <div className='page-guard-cover'>
      <div className='page-guard'>
        <div className='page-guard-header'>
          {!goodDate || !goodGuests ? <h4>Select a booking date first</h4> : <h4>Select a room first</h4>}
        </div>

        <div className='page-guard-body'>
          <Link className='page-guard-btn' to={!goodDate || !goodGuests ? '/calendar' : '/rooms'}>
            proceed
          </Link>
        </div>
      </div>
